...
- [x] (2026-02-02 13:12Z) Updated `backend/spec/submission-manager.md` to reference `backend/spec/manager-sync-timeout.md`.
...
- Decision: End the wait when the intent becomes terminal or when the first attempt completes (attempt_count >= 1), whichever comes first.
  Rationale: Matches the spec in `backend/spec/manager-sync-timeout.md` and keeps sync behavior transport-only.
  Date/Author: 2026-02-02 / Codex
...
Update `backend/spec/submission-manager.md` to mention the `waitSeconds` query parameter on `POST /v1/intents` and explicitly point readers to `backend/spec/manager-sync-timeout.md` for detailed sync-wait semantics. No behavior changes elsewhere.
...
- File: `backend/spec/submission-manager.md`
  - In the HTTP API section for `POST /v1/intents`, note the optional `waitSeconds` query parameter and reference `backend/spec/manager-sync-timeout.md` for full semantics.
- Confirm `backend/spec/manager-sync-timeout.md` remains the canonical detailed description and keep it unchanged unless clarifications are needed.
