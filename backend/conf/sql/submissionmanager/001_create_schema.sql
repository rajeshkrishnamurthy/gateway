...
IF OBJECT_ID('dbo.intent_delivery_webhook_ingestion', 'U') IS NULL
BEGIN
  CREATE TABLE dbo.intent_delivery_webhook_ingestion (
    ingestion_id BIGINT IDENTITY(1,1) NOT NULL PRIMARY KEY,
    source_record_id NVARCHAR(200) NOT NULL,
    intent_id NVARCHAR(200) NOT NULL,
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = 'uq_intent_delivery_webhook_ingestion_source_record'
    AND object_id = OBJECT_ID('dbo.intent_delivery_webhook_ingestion')
)
BEGIN
