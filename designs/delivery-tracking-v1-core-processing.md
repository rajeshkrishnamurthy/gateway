...
- This design therefore assumes direct transactional apply from accepted correlated records, with idempotent no-op handling on replay/duplicate apply attempts.
...
- Core input is a normalized correlated record with:
  - `sourceRecordId` (stable ingestion record id; required, generated by Setu at ingestion persistence boundary)
